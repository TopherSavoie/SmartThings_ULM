 ...
 *
 *  Copyright 2017 Chris Savoie (Based on User Lock Manafger Erik Thayer)
...
/*
 *	11/30/2017 >>> v0.4.2.20171130 - First Version
 *
 */
...
definition(
  ...
  namespace: "skorpian23",
  author: "Chris Savoie",
  description: "This app allows you to change, delete, and schedule user access. Based on Erik Thayer version",
...
def keypadPage() {
  dynamicPage(name: "keypadPage",title: "Keypad Settings (optional)") {
    section("Settings") {
      ...
      input(name: "keypadstatus", title: "Send Exit Delay status to keypad?", type: "bool", multiple: false, required: true, defaultValue: true)
      input(name: "keypadstatusSHM", title: "Update Keypad based on SHM status change?", type: "bool", multiple: false, required: true, defaultValue: true)
      input(name: "keypadschangeSHM", title: "Change SHM status based on Keypad event?", type: "bool", multiple: false, required: true, defaultValue: false)
      input(name: "keypadbeep", title: "Beep on SHM status change?", type: "bool", multiple: false, required: true, defaultValue: true)
    }
    def hhPhrases = location.getHelloHome()?.getPhrases()*.label
    hhPhrases?.sort()
    section("Routines") {
      ...
      input(name: "armDelay", title: "Arm Delay for Armed mode (in seconds)", type: "number", required: true, defaultValue:"30")
      input(name: "armDelayStay", title: "Arm Delay for Stay mode (in seconds)", type: "number", required: true, defaultValue:"0")
      input(name: "armDelayNight", title: "Arm Delay for Night mode (in seconds)", type: "number", required: true, defaultValue:"0")
...
private execRoutine(armMode) {
  if (armMode == 'away') {
    ...
    location.helloHome?.execute(settings.nightRoutine)
  }
}
    ...
    if (codeEntered == correctCode) {
      ...
      if (state."userState${i}".enabled == true) {
        ...
        if (data == "0") {
          ...
          message = "${evt.displayName} disarmed by ${unlockUserName}"
        }
        else if (data == "1") {
          //log.debug "sendStayCommand"
          if(armDelayStay && keypadstatus) {
            keypad?.each() { it.setExitDelay(armDelayStay) }
          }
          message = "${evt.displayName} set to 'Stay/Home' by ${unlockUserName}"
          if(armDelayStay) {
            message += ". Armed in ${armDelayStay} second."
          }
          runIn(armDelayStay, "sendStayCommand")
        }
        else if (data == "2") {
          //log.debug "sendNightCommand"
          if(armDelayNight && keypadstatus) {
            keypad?.each() { it.setExitDelay(armDelayNight) }
          }
          message = "${evt.displayName} set to 'Night' by ${unlockUserName}"
          if(armDelayNight) {
            message += ". Armed in ${armDelayNight} second."
          }
          runIn(armDelayNight, "sendNightCommand")
        }
        else if (data == "3") {
          ...
          message = "${evt.displayName} set to 'Away' by ${unlockUserName}"
          if(armDelay) {
            message += ". Armed in ${armDelay} second."
          }  
